@php
    ...
    $icons = [
        ...
        'email' => '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2.5" y="4.5" width="19" height="15" rx="3"/><path d="m3.5 7 7.4 5.3a2 2 0 0 0 2.2 0L20.5 7"/></svg>',
        'phone' => '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M7 3.5h2.2l1.4 4-1.9 1.4a12 12 0 0 0 5.4 5.4l1.4-1.9 4 1.4V16c0 2-1.6 3.4-3.5 3.2A15.5 15.5 0 0 1 4.3 7C4.1 5.1 5.4 3.5 7 3.5Z"/></svg>',
    ];
@endphp

@if ($dock)
    <div class="dock" data-dock>
        <ul class="dock__list" id="dock-channels">
            @foreach ($dock as $item)
                <li class="dock__item" style="--i: {{ $loop->index }}">
                    <a
                        class="dock__link dock__link--{{ $item['icon'] }}"
                        href="{{ $item['url'] }}"
                        @if (! str_starts_with($item['url'], 'mailto:') && ! str_starts_with($item['url'], 'tel:'))
                            target="_blank" rel="noopener"
                        @endif
                    >
                        <span class="dock__label">{{ $item['label'] }}</span>
                        <span class="dock__icon">{!! $icons[$item['icon']] ?? $icons['email'] !!}</span>
                    </a>
                </li>
            @endforeach
        </ul>
        ...
        >
            ...
            <svg class="dock__toggle-icon dock__toggle-icon--close" viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" aria-hidden="true">
