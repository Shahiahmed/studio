@php
    ...
    $submitLabel = $submitLabel ?? 'Обсудить проект';
@endphp
    ...
    <div class="lead-form__fields">
        <div class="lead-form__row">
            <div @class(['field', 'has-error' => $errors->has('name')]) data-field="name">
                <label class="field__label" for="{{ $prefix }}-name">Имя</label>
                <input class="field__input" id="{{ $prefix }}-name" name="name" type="text" autocomplete="name" placeholder="Как к вам обращаться" value="{{ old('name') }}" required maxlength="120" aria-describedby="{{ $prefix }}-name-error">
            ...
            <div @class(['field', 'has-error' => $errors->has('contact')]) data-field="contact">
                <label class="field__label" for="{{ $prefix }}-contact">Контакт</label>
                <input class="field__input" id="{{ $prefix }}-contact" name="contact" type="text" autocomplete="tel" placeholder="Телефон, Telegram или email" value="{{ old('contact') }}" required maxlength="190" aria-describedby="{{ $prefix }}-contact-error">
        ...
        <div @class(['field', 'has-error' => $errors->has('message')]) data-field="message">
            <label class="field__label" for="{{ $prefix }}-message">Описание задачи</label>
            <textarea class="field__input" id="{{ $prefix }}-message" name="message" rows="4" maxlength="5000" placeholder="Что нужно сделать, какие есть сроки и пожелания" aria-describedby="{{ $prefix }}-message-error">{{ old('message') }}</textarea>
